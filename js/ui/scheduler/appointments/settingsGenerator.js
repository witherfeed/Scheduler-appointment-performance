...
export class DateGeneratorBaseStrategy {
    ...
    generate(appointmentAdapter) {
        ...
        let appointmentList = this._createAppointments(appointmentAdapter, itemGroupIndices); // 4
        window.createAppointments += Date.now() - dateNow;

        appointmentList = this._getProcessedByAppointmentTimeZone(appointmentList, appointmentAdapter); // T983264 //1

        if(this._canProcessNotNativeTimezoneDates(appointmentAdapter)) {
            appointmentList = this._getProcessedNotNativeTimezoneDates(appointmentList, appointmentAdapter); // 1
        }

        dateNow = Date.now();
        let dateSettings = this._createGridAppointmentList(appointmentList, appointmentAdapter); // 5
        window.createGridAppointmentList += Date.now() - dateNow;

        dateNow = Date.now();
        dateSettings = this._cropAppointmentsByStartDayHour(dateSettings, this.rawAppointment); // 14
        window.cropAppointmentsByStartDayHour += Date.now() - dateNow;

        dateSettings = this._fillNormalizedEndDate(dateSettings, this.rawAppointment); // 1
    ...
    _createAppointments(appointment, groupIndices) {
        ...
        if(appointment.isRecurrent) {
            appointments = this._createRecurrenceAppointments(appointment, groupIndices);
        } else {
            appointments.push({
    ...
    _createExtremeRecurrenceDates() {
        const startViewDate = this.appointmentTakesAllDay
    ...
    _createRecurrenceAppointments(appointment, groupIndices) {
        ...
        const result = generatedStartDates.map(date => {
